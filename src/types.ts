...
const importedBooks: Book[] = [...books];
...
function searchBooks(query: string): Book[] {
  ...
  const filtredBooks = importedBooks.filter(
    (book) =>{
...
function getbookByISBN(ISBN: string) {
  const book: Book | null = importedBooks.filter(book => book.ISBN).find(book => book.ISBN === ISBN) || null
  return book;
}
